...
curl -F file=@policies_v2/demo_pol_3_perf_isolation_different_ns-slice_1-robot.yml http://localhost:5000/iml/update-policy

echo "Applying reduced resource usage policy to the non-important robot traffic in network slice 1..."
curl -F file=@policies_v2/demo_pol_3_perf_isolation_different_ns-slice_1-bg.yml http://localhost:5000/iml/update-policy

echo "Applying reduced resource usage policy to the simulated traffic in network slice 2..."
curl -F file=@policies_v2/demo_pol_3_perf_isolation_different_ns-slice_2.yml http://localhost:5000/iml/update-policy
